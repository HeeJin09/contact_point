...
arrow = p.loadURDF("abc.urdf",[0.0, 0.0, 0.15],quat)
...
while True:
    ...
    # for i in range(3):
    #     if(abs(filter_Wrench[i]) < 0.005):
    #         result_Wrench[i] =0
    #     else:
    #          result_Wrench[i] = filter_Wrench[i]
    # for i in range(3):
    #     if(abs(result_Wrench1[i+3]) < 1.1):
    #         result_Wrench[i+3] =0
    #     else:
    #          result_Wrench[i+3] = result_Wrench1[i+3]
    ...
    try:
        ...
        if r1[0] > 0.08 or r1[0] < -0.08 or r1[1] > 0.08 or r1[1] < -0.07 or r1[2] > 0.145  or r1[2] < 0.145 : r1[0] = [0,0,0] 
    ...
    try:
        ...
        if r2[0] > 0.075 or r2[0] < -0.075 or r2[1] > -0.06 or r2[1] < -0.6 or r2[2] > 0.146 or r2[2] < 0.005  : r2 = [0,0,0]
    except:
        r2 = [0,0,0]
        
    try:
        r3 = np.linalg.inv(U3.T @ U3)@U3.T @np.array([-result_Wrench[0]/norm_f,-result_Wrench[1]/norm_f,-result_Wrench[2]/norm_f,-d3 ])
        if r3[0] > 0.075 or r3[0] < -0.075 or r3[1] > 0.061 or r3[1] < 0.059  or r3[2] > 0.146 or r3[2] < 0.005: r3 = [0,0,0]
    except:
        r3 = [0,0,0]	

    try:
        r4 = np.linalg.inv(U4.T @ U4)@U4.T @np.array([-result_Wrench[0]/norm_f,-result_Wrench[1]/norm_f,-result_Wrench[2]/norm_f,-d4 ])
        if r4[0] < -0.061 or r4[0] > -0.059 or r4[1] > 0.07 or r4[1] < -0.07 or r4[2] > 0.146 or r4[2] < 0.025 : r4 = [0,0,0]
    except:
        r4 = [0,0,0]

    try:
        r5 = np.linalg.inv(U5.T @ U5)@U5.T @np.array([-result_Wrench[0]/norm_f,-result_Wrench[1]/norm_f,-result_Wrench[2]/norm_f,-d5 ])
        if r5[0] > 0.061 or r5[0] < 0.059 or r5[1] > 0.07 or r5[1] < -0.07 or r5[2] > 0.146 or r5[2] < 0.025 : r5 = [0,0,0]
    except:
        r5 = [0,0,0]

    ########## determine r from force vector ###########
    f_nx = np.dot(u,nx)*nx*norm_f*0.015
    f_ny = np.dot(u,ny)*ny*norm_f*0.015
    f_nz = np.dot(u,nz)*nz*norm_f*0.015
    ...
    roll = np.arctan2(u[2], u[1])
    pitch = np.arctan2(u[0], np.sqrt(u[1]**2 + u[2]**2))
    yaw = 0.0
    test_quat = p.getQuaternionFromEuler([roll,pitch,yaw])
    ...
    if norm_f < 1 or (r[0]==0 and r[1]==0 and r[2]==0):
        r = np.array([nan,nan,nan])
    if count_Test_1 % 20 == 0:
        print(r)
    ...
    yaw = 0.0
    test_quat = p.getQuaternionFromEuler([roll,pitch,yaw])
    position = np.array(R@ r.T)+ np.array(R@ np.array([0.00, 0.00, 0.02]).T)
    p.resetBasePositionAndOrientation(arrow, position, test_quat)    
